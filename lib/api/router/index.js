	...
	constructor(expressApp, db) {
		...
		expressApp.get('/authors', (incomingMessage) => {
			...
			db.query('SELECT id,name FROM authors')
				...
				.then(() => {
					const list = authors.map((author) => `SELECT id,book_id,author_id FROM library WHERE author_id = ${author.id}`);
					return db.queryAll(list);
				})
				.then((results) => {
					const rows = results.map((result) => result.rows);
					...
					authors.forEach((author, index) => {
						const authorRows = rows[index];
						authorRows.forEach((bookRow) => {
							if (author.id === bookRow['book_id']) {
								author.books.push(bookRow['author_id']);
							}
						});
					});

					incomingMessage.res.send(authors);
				}, (err) => incomingMessage.res.send(err));
		});
